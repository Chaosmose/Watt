...
- (instancetype)wattCopyInRegistry:(WattRegistry*)registry{
    WattCollectionOfUser *instance=[super copy];
    [registry addObject:instance];
     WattRegistry *__block registryReference=registry;
    [self enumerateObjectsUsingBlock:^(WattUser *obj, NSUInteger idx, BOOL *stop) {
...
- (id)copyWithZone:(NSZone *)zone{
    WattCollectionOfUser*instance=[[[self class] allocWithZone:zone] init];
    instance->_registry=nil; // We want to furnish a registry free copy
    //_uinstID=0;// we do not provide an _uinstID
   WattCollectionOfUser*__block ref=instance;
    [self enumerateObjectsUsingBlock:^(WattUser *obj, NSUInteger idx, BOOL *stop) {
...
- (void)enumerateObjectsUsingBlock:(void (^)(WattUser *obj, NSUInteger idx, BOOL *stop))block reverse:(BOOL)useReverseEnumeration{
    ...
    for( WattUser* obj in enumerator ){
        block(obj, idx++, &stop);
        if( stop )
            break;
    }
...
- ( WattCollectionOfUser*)filteredCollectionUsingBlock:(BOOL (^)(WattUser  *obj))block withRegistry:(WattRegistry *)registry{
	 WattCollectionOfUser *__block collection=[[WattCollectionOfUser alloc] initInRegistry:registry];
	    [self enumerateObjectsUsingBlock:^(WattUser *obj, NSUInteger idx, BOOL *stop) {
	        if(block(obj)){

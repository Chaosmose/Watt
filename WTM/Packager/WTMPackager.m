...
@interface WTMPackager()<SSZipArchiveDelegate>{
...
+ (WTMPackager*)sharedInstance {
    static WTMPackager *sharedInstance = nil;
...
-(void)packWattBundleWithName:(NSString*)name
                    withBlock:(void (^)(BOOL success))block{
    NSString *sourceFolderPath=[wtmAPI absolutePathForRegistryBundleWithName:name];
    ...
    NSInteger i=0;
    while ([wtmAPI.fileManager fileExistsAtPath:destinationFilePath]) {
...
-(void)unPackWattBundleWithName:(NSString*)name
    ...
    NSString *p=[[[wtmAPI absolutePathForRegistryBundleWithName:name] lastPathComponent] stringByReplacingOccurrencesOfString:@"/" withString:@""];
    ...
    NSString *destinationFolder=[[wtmAPI applicationDocumentsDirectory] stringByAppendingString:folderName];
    if(![wtmAPI.fileManager fileExistsAtPath:zipSourcePath]){
        zipSourcePath=[[wtmAPI applicationDocumentsDirectory] stringByAppendingString:[zipSourcePath lastPathComponent]];
    }
    if(![wtmAPI.fileManager fileExistsAtPath:zipSourcePath]){
        WTLog(@"%@ do not exist",zipSourcePath);
    }else{
        [wtmAPI createRecursivelyRequiredFolderForPath:destinationFolder];
...
-(void)unZip:(NSString*)zipSourcePath
   ...
   withBlock:(void (^)(BOOL success))block{
    ...
    if([wtmAPI.fileManager fileExistsAtPath:zipSourcePath]){
        if([wtmAPI createRecursivelyRequiredFolderForPath:destinationFolder]){
...
-(void)zip:(NSString*)sourcePath
 ...
 withBlock:(void (^)(BOOL success))block{
    NSError*error=nil;
    if([wtmAPI.fileManager fileExistsAtPath:sourcePath]){
        if([wtmAPI.fileManager fileExistsAtPath:destinationZipFilePath]){
            [wtmAPI.fileManager removeItemAtPath:destinationZipFilePath error:&error];
        }
#if TARGET_OS_IPHONE
...
-(void)_addFolder:(NSString*)path pathPrefix:(NSString*)prefix toMapping:(NSMutableDictionary**)dictionary{
    NSFileManager *fm=wtmAPI.fileManager;
